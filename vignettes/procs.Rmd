...
the weighted values.  If you want datasets returned from the function,
pass the "out" or "report" keyword to the `options` parameter.  There are many more
options to control the output data and display.  Note that the "noprint" option
has been added to these examples to allow the **procs** package to pass
CRAN checks.  To see the function output in the viewer, 
remove this option.
...
res <- proc_freq(dt, tables = v(Hair, Eye, Hair * Eye),
                 weight = Freq,
                 options = v(report, chisq, noprint),
...
values.  Note that any class groupings are nested in the by.  Like the 
`proc_freq()` function, you may request output datasets using the "out"
option.
