...
The function call requested that all the report datasets
be output in a list.  These datasets can be seen below:
```{r eval=FALSE, echo = TRUE}
# View output datasets
res
# $Hair
#     CAT   N CNT      PCT
# 1 Black 592 108 18.24324
# 2 Blond 592 127 21.45270
# 3 Brown 592 286 48.31081
# 4   Red 592  71 11.99324
# 
# $Eye
#     CAT   N CNT      PCT
# 1  Blue 592 215 36.31757
# 2 Brown 592 220 37.16216
# 3 Green 592  64 10.81081
# 4 Hazel 592  93 15.70946
# 
# $`Hair * Eye`
...
res2 <- proc_means(dt, var = Freq, 
                   ...
                   out1 = out(where = expression(TYPE == 1),
                              drop = v(TYPE, FREQ, VAR)))
...
res2
#       BY CLASS N  MEAN       STD MIN MAX
# 1 Female Black 4 13.00 15.599145   2  36
# 2 Female Blond 4 20.25 29.216149   4  64
# 3 Female Brown 4 35.75 21.884165  14  66
# 4 Female   Red 4  9.25  4.500000   7  16
# 5   Male Black 4 14.00 12.516656   3  32
# 6   Male Blond 4 11.50 12.503333   3  30
# 7   Male Brown 4 35.75 18.679311  15  53
# 8   Male   Red 4  8.50  1.732051   7  10
```

Note that both `proc_freq()` and `proc_means()` follow the convention of naming
columns according to the statistic or parameter they represent.  This convention
is somewhat different from SAS®, but makes it easier when you need to manipulate
the output dataset.

Also observe that there are some additional parameters on the `out()` request
that allow you to do shaping of the output dataset.  These same 
shaping parameters are available on `proc_freq()` as well.
...
# Tranpose statistics to rows
res3 <- proc_transpose(res2, id = CLASS, by = BY, name = Statistic)
...
res3$BY <- factor(res3$BY, levels = c("Male", "Female"))

# Sort male to top
res4 <- proc_sort(res3, by = BY)
#        BY Statistic    Black    Blond    Brown       Red
# 6    Male         N  4.00000  4.00000  4.00000  4.000000
# 7    Male      MEAN 14.00000 11.50000 35.75000  8.500000
# 8    Male       STD 12.51666 12.50333 18.67931  1.732051
...
fmt <- flist(STD = "%.3f", type = "row", lookup = res4$Statistic)
...
# Assign formats 
formats(res4) <- list(Statistic = vf,
                      Black = fmt,
                      Blond = fmt,
                      Brown = fmt,
                      Red = fmt)
...
# Assign labels
labels(res4) <- list(stub = "Sex")

# Create list for reporting
prnt <- list(res$`Hair * Eye`, res$Chisq, res4)
...
* [Other Functions](procs-other.html)
