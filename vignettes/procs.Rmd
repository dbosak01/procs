...
The **procs** package brings the concepts of data procedures, data 
dictionaries, and data steps to R.  

* A **data procsary** is an object used to define 
and manage an entire directory of data files.  
* A **data dictionary** is a data 
frame full of information about a data procsary, data frame, or tibble. 
...
The **procs** package also includes an enhanced equality operator to
make data comparisons more intuitive.

### Key Functions

The above concepts are implemented in the **procs** package with four key 
functions. They are:

* `libname()`: Creates a data procsary
...
in the **procs** package, and also available for download 
[here](https://github.com/dbosak01/procs/blob/master/inst/extdata/procs_data.zip).

#### The `libname()` Function
The **procs** `libname()` function is quite similar to the SAS® `libname`
statement.  The first parameter is the name of the procsary.  The 
second parameter is a path to a directory the procsary will point to.  The 
third parameter is the engine with which to read and write the data.  
```{r eval=FALSE, echo=TRUE} 
procsary(procs)

# Get path to sample data
pkg <- system.file("extdata", package = "procs")

# Define data procsary
...
* A summary print-out of the procsary 

The summary print-out looks like this:
```
# procsary 'sdtm': 8 items
- attributes: csv not loaded
- path: C:/packages/procs/inst/extdata
- items:
...
The summary displays what type of procsary it is, where it is located,
and what data (if any) is already in the procsary directory.  In this case,
...
**procs** `libname()` function.  The difference is that after the 
SAS® `libname` statement is called, the data is immediately available to
your code using two-level (\<procsary\>.\<dataset\>) syntax.  

With the **procs** function, on the other hand, the data is immediately 
available using list syntax on the procsary variable name.  To get the 
...
Notice on the console printout that the procsary is now "loaded".  That means
...
pane will now show all the datasets available in the procsary.
...
To see more examples of the **procs** data management 
functions, refer to the articles on [Basic procsary Operations](procs-basics.html)
and [procsary Management](procs-management.html).

#### The `dictionary()` Function
Once you have a procsary defined, you may want to examine 
the column attributes for the datasets in that procsary.
Examining those column attributes can be accomplished with the `dictionary()` 
function. The `dictionary()`
function returns a tibble of information about the data in the procsary.

Continuing from the example above, let's look at the dictionary for the 
'sdtm' procsary created previously.
```{r eval=FALSE, echo=TRUE}
dictionary(sdtm)
# # A tibble: 130 x 10
...
the **procs** dictionary table is overall quite similar to a SAS® dictionary
...
The **procs** package offers a `datastep()` function that simulates this style
...
Here is a simple example, again using the data from the procsary already 
defined above:
...
help page and in the data step [article](procs-datastep.html).
...
The `%eq%` function contained in the **procs** packages attempts to provide
...
documentation, or in the [Enhanced Equality](procs-eq.html) article.
...
* [Proc Freq](procs-freq.html)
* [Proc Means](procs-means.html)
* [Proc Transpose](procs-transpose.html)
* [Complete Example 1](procs-example1.html)
