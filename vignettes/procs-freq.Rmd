...
an interactive function and can return datasets.  Unless you ask
for output data, the function is interactive only.
...
```
Notice that the way output datasets are requested from `proc_freq` is much
...
data is organized.  The options are best illustrated by an example:
```{r eval=FALSE, echo=TRUE} 
# Shape wide
