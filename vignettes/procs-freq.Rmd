...
To better understand your data, it is important to visualize it.  The 
`proc_freq()` function offers basic plotting of frequencies via the 
"plots" parameter and the `freqplot()` function.  Here is a simple example 
demonstrating how to get frequency plots for a selection of tables
...
The report now shows a bar chart for each of the table requests. The style 
of plot can be modified using the parameters on the `freqplot()` function,
like this:
...
Modifying the `freqplot()` parameters can help improve the quality of the 
