...
You may request datasets from the `proc_freq` function using the `output`
parameter.  This parameter takes three values: "all", "report", and "none".
The "all" keyword requests datasets meant for output, and is the default.
...
simpler than the corresponding mechanism in SAS®.  With `proc_freq`, by default,
all requested tables and statistics will be
