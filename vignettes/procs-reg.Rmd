...
```
The `proc_reg()` function simulates a SAS® PROC REG procedure.  It is used to
perform a regression analysis.  The function is both 
interactive and returns datasets.   

## Create Sample Data

First, let's create some sample data.  This sample data shows Sex, Age, Height,
and Weight for a group of students:  
...
* **Observations**:  The number of used and unused observations in the input 
...
* **Fitness**: The Fitness table shows measures related to fitness, such as the
Root MSE, R-Square value, and Adjusted R-Square.
* **Coefficients**: The Coefficients table includes parameter estimates for 
...
you do that using `output = "report"`:
